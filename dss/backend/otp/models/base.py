...
class Device(models.Model):
    ...
    .. attribute:: users
        ...
        *ManyToManyField*: Many to many relationship with your user model, as configured by
    ...
    users = models.ManyToManyField(
        ...
        blank=True,
        null=True,
        help_text="The users that this device belongs to.",
...
class SideChannelDevice(Device):
    ...
    def verify_token(self, token):
        ...
        _now = timezone.now()
        _token = token if isinstance(token, str) else str(token)
        if (
            (self.token is not None)
            and (_token == self.token)
